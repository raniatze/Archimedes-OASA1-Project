...
def make_dataset(csv_file):
    ...
    with open(updated_csv_file, 'w') as output_file:
        ...
        headers = pd.DataFrame(columns=['Line_descr', 'Direction', 'Stop_id', 'Stop_order', 'Minute_of_day', 'Day_of_month', 'Day_of_week', 'Week_of_year', 'Day_of_year', 'Is_holiday', 'Temperature', 'Precipitation', 'T_pa_in_veh', 'Unique_hash'])
        ...
        for name, group in grouped:
            ...
            if unique_line_descr[0] not in s_route_ids:
                s_route_ids[unique_line_descr[0]] = s_value_route_id.value
                #print(unique_line_descr[0], value_route_id)
                line_to_write = unique_line_descr[0] + ': ' + str(s_value_route_id.value)
                append_to_file('Line_descr_encodings', line_to_write)
                s_value_route_id.value += 1
            ...
            for stop in unique_stop_id:
                if stop not in s_stop_ids:
                    s_stop_ids[stop] = s_value_stop.value
                    line_to_write = str(stop) + ': ' + str(s_value_stop.value)
                    append_to_file('Stop_id_encodings', line_to_write)
                    s_value_stop.value += 1

            group_sorted['Custom_stop_id'] = group_sorted['Stop_id'].replace(s_stop_ids)
            ...
            data = {
                'Line_descr': s_route_ids[unique_line_descr[0]],
