...
def make_dataset(csv_file):
    ...
    if not os.path.isfile(target):
        ...
        with open(target, 'w',newline='') as output_file:
            ...
            headers = pd.DataFrame(columns=['Name','Stop_order','Arrival_datetime','Type_intp'])
            ...
            for name, group in grouped:
                ...
                group_sorted = group_sorted.reset_index()
                group_sorted[['Line_descr','S_order','Arrival_datetime','Type_intp']].to_csv(target, mode='a', index=False, header=False)
